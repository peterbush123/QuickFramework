...
//游戏操作网络视图
@ccclass("EliminateGridView")
export default class EliminateGridView extends EventComponent {
    ...
    private initListener() {
        this.onN(this.node,Node.EventType.TOUCH_START, this.onTouchStart);
        this.onN(this.node,Node.EventType.TOUCH_MOVE, this.onTouchMove);
    }
    ...
    updateView(changeModels: EliminateCellModel[]) {
        ...
        for (let i in changeModels) {
            ...
            if (!viewInfo) {
                ...
                let cache = App.cache.get(this.view.bundle, prefabUrl,Prefab);
                if( cache ){
