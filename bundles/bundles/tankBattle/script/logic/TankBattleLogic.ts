import { EventKeyboard, KeyCode, macro, Node, Sprite } from "cc";
...
export class TankBattleLogic extends Logic {
    ...
    protected _view: TankBattleGameView = null!;
    get view(){
        return this._view;
    }
    ...
    onOpenSlectedView() {
        ...
        App.uiManager.open({ type: TankBattleStartView, bundle: this.module, zIndex: ViewZOrder.UI , args: [this] });
    }

    /**@description 打开过渡切换游戏关卡界面 */
    onOpenChangeView() {
        this.data.gameStatus = TankBettle.GAME_STATUS.INIT;
        App.uiManager.open({ type: TankBattleChangeStageView, bundle: this.module, zIndex: ViewZOrder.UI , args: [this.data.currentLevel, this] })
    }
    ...
    updateGameInfo() {
        ...
        if (this.view) {
            this.view.showGameInfo(gameInfo);
        }
    }
    ...
    showMap(level: number) {
        ...
        if (this.view && this.mapCtrl) {
            ...
            this.view.audioHelper.playMusic(TankBettle.AUDIO_PATH.START)
        }
    }
    ...
    gameOver() {
        if (this.mapCtrl) {
            ...
            App.uiManager.open({ type: TankBattleGameOver, bundle: this.module, zIndex: ViewZOrder.UI , args: [this] });
    ...
    loadImage(sprite: Sprite | null, spriteFrameKey: string) {
        if (this.view && sprite) {
            sprite.loadImage({ url: { urls: ["texture/images"], key: spriteFrameKey }, view: this.view, bundle: this.module });
        }
    }
    ...
    playEffect( url : string ){
        if ( this.view ){
            this.view.audioHelper.playEffect(url);
        }
    }

    onShowMapLevel( level : number ){
        App.uiManager.close(TankBattleStartView);
        this.showMap(level);
    }
    ...
    onKeyUp(ev: EventKeyboard, type : TankBettle.ViewType , view ?: TankBattleStartView ) {
        if ( type == TankBettle.ViewType.GAME_VIEW ){
            ...
            if (ev.keyCode == KeyCode.KEY_N) {
                //手动下一关，恢复下生命
                this.data.isSingle = this.data.isSingle;
                this.nextLevel();
            } else if (ev.keyCode == KeyCode.KEY_P) {
                //手动下一关，恢复下生命
                this.data.isSingle = this.data.isSingle;
                this.prevLevel();
            ...
            if (ev.keyCode == KeyCode.ARROW_DOWN || ev.keyCode == KeyCode.ARROW_UP ) {
                let isSingle = false;
                if (view.isSingle()) {
            ...
            }else if( ev.keyCode == KeyCode.SPACE || ev.keyCode == KeyCode.ENTER ){
                this.data.isSingle = this.data.isSingle;
                this.onOpenChangeView();
            }
        }
