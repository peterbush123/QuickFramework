...
export class TaxiCarMgr extends EventComponent {
    ...
    public get camera(){
        if ( !this._camera ){
            let origin = App.uiManager.camera3d.node;
            origin.active = false;
            this._camera = instantiate(App.uiManager.camera3d.node);
    ...
    addEvents(){
        super.addEvents();
        this.onD(TaxiConstants.EventName.GAME_START,this._gameStart);
        this.onD(TaxiConstants.EventName.GAME_OVER,this._gameOver);
    }
    ...
    private _createMainCar(point: Node) {
        ...
        if ( !this.pools.has(name) ){
            let pool = App.pool.createPool(name) as NodePool;
            let data = App.cache.get(TaxiData.module,`prefabs/car/${name}`,Prefab)?.data;
