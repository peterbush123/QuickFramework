...
export class TaxiCustomerMgr extends EventComponent {
    ...
    init(){
        let root = App.uiManager.root3D;
        for ( let i = 1 ; i <=2 ; i++ ){
            let url = `prefabs/customer/customer0${i}`;
            let data = App.cache.get(this.data.module,url,Prefab);
    ...
    addEvents(){
        super.addEvents();
        this.onD(EventName.GREETING, this._greetingCustomer);
        this.onD(EventName.GOODBYE, this._takingCustomer);
    }
    ...
    private _greetingCustomer(ev:DispatchEvent,...args: any[]){
    ...
    private _takingCustomer(ev:DispatchEvent,...args: any[]) {
