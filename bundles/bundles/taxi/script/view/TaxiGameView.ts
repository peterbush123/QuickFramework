...
export class TaxiGameView extends GameView {
    ...
    @inject({type : TaxiData.module , name : "logic"})
    protected logic : TaxiLogic = null!;

    @inject({type : TaxiData.module , name : "data"})
    protected data : TaxiData = null!;
    ...
    init() {
        ...
        this.onN(find("getBtnNormal/btnNormal",this.result.node)!,Node.EventType.TOUCH_END,this.onContinue);
        ...
        this.onN(this.game.node,Node.EventType.TOUCH_START, this.onTouchStart);
        this.onN(this.game.node,Node.EventType.TOUCH_END, this.onTouchEnd);
        this.onN(this.main.node,Node.EventType.TOUCH_END, this.onGameStart);
    ...
    addEvents() {
        super.addEvents();
        this.onD(TaxiConstants.EventName.GREETING, this.onGreeting);
        this.onD(TaxiConstants.EventName.GOODBYE, this.onGoodbye);
        this.onD(TaxiConstants.EventName.SHOW_TALK, this.onTalking);
        this.onD(TaxiConstants.EventName.SHOW_GUIDE, this.onShowGuide);
    }
    ...
    private onTalking(ev:DispatchEvent,customerID:number) {

        const table = this.data.getLanguage("tips" as any);
    ...
    private onShowGuide(ev:DispatchEvent,isShow : boolean) {
    ...
    onShow(): void {
        this.audioHelper.playMusic(TaxiConstants.AudioSource.BACKGROUND)
        super.onShow();
    }
}
