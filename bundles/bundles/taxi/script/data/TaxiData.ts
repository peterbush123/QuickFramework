...
import { TaxiLan } from "./TaxiLanguage";
...
/**@description 大厅数据 */
export class TaxiData extends GameData<typeof TaxiLan.data> {
    ...
    init() {
        const info = App.storage.getItem(TaxiConstants.PlayerConfigID);
        if ( info ){
    ...
    public savePlayerInfoToCache(){
       ...
       App.storage.setItem(TaxiConstants.PlayerConfigID, data);
    }
}
